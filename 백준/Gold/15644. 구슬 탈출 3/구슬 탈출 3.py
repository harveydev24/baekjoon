...
def move(red_r, red_c, blue_r, blue_c, i, cnt, lst):
...
def bfs():
    ...
    while q:
        red_r, red_c, blue_r, blue_c, cnt, lst = q.popleft()
        if cnt >= 10:
            break
        ...
        for i in range(4):
            if i in direction:
                tmp = move(red_r, red_c, blue_r, blue_c, i, cnt, lst+[i])
                ...
                if visited.get((red_r_, red_c_, blue_r_, blue_c_)) == None:
                    ...
                    q.append((red_r_, red_c_, blue_r_,
                             blue_c_, cnt+1, lst+[i]))
